...
```json
{
  ...
  "packages": {
    ...
    "node_modules/@replit/vite-plugin-cartographer": {
      ...
      "integrity": "sha512-C1RRIDqaFtfP6BSztgCJBPA+hSH3bUNc3xbr/zzVOwOdZLJy0VaM6ypN9bwWbzksGmEZpdLQVvCAG0LLwCwCpQ==",
      "dev": true,
      "dependencies": {
        "@babel/parser": "^7.26.9",
...
```
 ...
 server/db/models/click.model.ts
...
```
 ...
 server/db/models/link.model.ts
...
```
 ...
 server/db/models/user.model.ts
...
(async () => {
  ...
  // In Vercel, use their PORT environment variable, otherwise default to 5000
  ...
  server.listen({
    ...
    if (process.env.MONGODB_URI) {
      log(`MongoDB is configured and will be used for storage`, "mongodb");
    } else {
      log(`MongoDB is not configured, using in-memory storage`, "mongodb");
    }
  });
})();
```
